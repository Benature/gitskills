...
var level_move = function(){
    ...
    ball.x += lor * v_x *fre*v_back + wind_v*fre*0.3;
    v_x -= airf;
    if (v_x <= 0) {v_x=0;}
...
var begin_interface = function(){
    context.drawImage(background_begin, 0, 0, 750,563,0,-200,1500,1126);
...
var end_interface = function(){
    ...
    context.fillText("得分: "+scoreboard,650,400);
...
var prop_wind = function(){
    ...
    //出现台风并持续
        ...
        context.fillStyle ="purple";
        context.font = "50px Arial";
        context.strokeText("中珠又刮台风辣!",200,60);
        context.fillText("中珠又刮台风辣!",200,60);
        context.font = "30px Arial";
        if (wind_v>0){
            context.strokeText("→"+wind_v+"级鸭大魔法飓风",600,50);
            context.fillText("→"+wind_v+"级鸭大魔法飓风",600,50);
        }
        if (wind_v<0){
            context.strokeText("←"+(-wind_v)+"级鸭大魔法飓风",600,50);
            context.fillText("←"+(-wind_v)+"级鸭大魔法飓风",600,50);
...
//board.left=false;board.right=false;board.up=false;board.down=false;
//board.left0=false;board.right0=false;board.up0=false;board.down0=false;//碰撞检测
boardheart.x = (boardbasic.x + 85.5*boardbasic.k)*boardbasic.lr;
boardheart.y=boardbasic.y + 240*boardbasic.k;//篮框中心 判断得分
basketry.x1 = (boardbasic.x + 10.5*boardbasic.k)*boardbasic.lr;basketry.y1=boardbasic.y + 240*boardbasic.k;//篮筐左端(近似为圆)圆心
basketry.x2=(boardbasic.x + 160.5*boardbasic.k)*boardbasic.lr;basketry.y2=boardbasic.y + 240*boardbasic.k;//篮筐右端(近似为圆)圆心
...
var collision = function(){
    context.scale(-1,1);
    context.drawImage(board_img, 0, 0, 600,600,boardbasic.x,Math.abs(boardbasic.y),Math.abs(600*boardbasic.k),Math.abs(600*boardbasic.k));
   //context.drawImage(board_column,0, 0,600,600,boardbasic.x,Math.abs(boardbasic.y),Math.abs(600*boardbasic.k),Math.abs(600*boardbasic.k));
    context.drawImage(board_column,0, 0,600,600,boardbasic.x,Math.abs(boardbasic.y),Math.abs(600*boardbasic.k),Math.abs(800*boardbasic.k));
...
var scoreyes = function(){
    ...
    if ((Math.abs(ball.y - boardheart.y) <= 3) && (Math.abs(ball.x - boardheart.x) <= 30) && v_y <= 0){
        boardbasic.x = -(Math.random()*1100 + 200);
...
ball_img.onload = function () {
    ...
    setInterval(function () {
        ...
        if (!gamebegin){begin_interface();t=0;scoreboard=0;}
            ...
            context.fillText("你的分数" + scoreboard,120,500);
        ...
        if (backdoor.count == 5){
            context.font = "20px Arial";
            context.fillStyle ="black";
            context.fillText("嘎吱~你打开了一扇后门!",1250,20);
        }
        context.restore();
    }, 1000 *1/ (60*3));
